...
class DepositCategoriesSerializer(ModelSerializer):
	class Meta:
		model = admin_models.PropertyDepositCategory
		fields = '__all__'

	def create(self, validated_data):
		validated_data['name'] = validated_data['name'].upper()

		if admin_models.PropertyDepositCategory.objects.filter(name=validated_data['name']).exists():
	...
	def update(self, instance, validated_data):
		...
		if admin_models.PropertyDepositCategory.objects.filter(name=validated_data['name']).exclude(pk=instance.pk).exists():
			raise ValidationError("A record with similar details already exists")
		return super().update(instance, validated_data)
	
class RecurrentChargesSerializer(ModelSerializer):
	class Meta:
		model = admin_models.PropertyReccurentPayment
		fields = '__all__'

	def create(self, validated_data):
		validated_data['name'] = validated_data['name'].upper()

		if admin_models.PropertyReccurentPayment.objects.filter(name=validated_data['name']).exists():
			raise ValidationError("A record with similar details already exists")
		return super().create(validated_data)
	...
	def update(self, instance, validated_data):
		...
		if admin_models.PropertyReccurentPayment.objects.filter(name=validated_data['name']).exclude(pk=instance.pk).exists():
			raise ValidationError("A record with similar details already exists")
		return super().update(instance, validated_data)